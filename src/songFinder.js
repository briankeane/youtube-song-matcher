...
class SongFinder {
  ...
  async getSongMatches({ artist, title, durationMS }) {
    ...
    return resultsRanker.rankAndSort(videos);
  }
  ...
  async getAutoGeneratedMatches({ artist, title, durationMS }) {
    ...
    const videos = flattenIDs(await this.getPlaylistItems({ id: channelDetails.contentDetails.relatedPlaylists.uploads.id }));
    return await this.addDetailsToVideos({ videos });
  }
